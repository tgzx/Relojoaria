...
create table if not exists public.banners (
  ...
  image_url text,
  storage_path text,
  theme_preset text not null default 'classic-night',
  image_position text not null default 'center center',
  image_fit text not null default 'cover' check (image_fit in ('cover', 'contain')),
  image_brightness numeric(4,2) not null default 0.92,
  image_contrast numeric(4,2) not null default 1.05,
  overlay_strength numeric(4,2) not null default 0.56,
  target_url text,
  target_type text,
  is_active boolean not null default true,
  sort_order integer not null default 0,
...
create index if not exists idx_banners_storage_path on public.banners (storage_path) where storage_path is not null;
...
-- Atomicidade da imagem principal de produto.
-- Mantem no maximo uma imagem principal por produto e oferece RPCs
-- transacionais para criar, trocar e excluir/promover imagens.
...
begin
  ...
  select pi.store_id, pi.is_primary into image_store_id, image_is_primary
...
begin
  ...
  insert into public.product_images (
    store_id, product_id, image_url, storage_path, alt_text, is_primary, sort_order
  ) values (
    product_store_id, target_product_id, target_image_url, target_storage_path,
    target_alt_text, not has_primary, coalesce(target_sort_order, 0)
  )
...
begin
  select pi.product_id into target_product_id
  from public.product_images pi
  where pi.id = target_image_id;
  ...
  return jsonb_build_object(
    ...
    'promoted_image', case when promoted_image.id is null then null else to_jsonb(promoted_image) end
  );
end;
$$;
